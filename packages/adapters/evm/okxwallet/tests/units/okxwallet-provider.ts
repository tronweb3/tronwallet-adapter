...
export class OkxWalletProvider extends EventEmitter {
    isOkxWallet = true;
...
export function installOkxWalletEIP6963Provider(
    provider: EIP1193Provider,
    options: { name?: string; rdns?: string } = {}
) {
    const detail = {
        info: {
            uuid: `${options.rdns || 'com.okex.wallet'}-${options.name || 'OKX Wallet'}`,
            name: options.name || 'OKX Wallet',
            icon: '',
            rdns: options.rdns || 'com.okex.wallet',
        },
        provider,
    };
